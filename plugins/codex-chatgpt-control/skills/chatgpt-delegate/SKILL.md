...
For a focused planning, research, reasoning, or second-opinion consultation,
use the bundled `chatgpt-gpt-5-6-high-consult` skill instead. It pins
GPT-5.6 Sol at High Intelligence, never falls back to Pro, and applies strict
response-completion gates.
...
Any configuration is a visible preference, not a guaranteed underlying model
identifier. Inspect the `configuration` sequence step and require
`verified: true` before claiming that a requested setting was active. If the
account does not expose that option, report the structured blocker and visible
candidates.
...
The legacy `mode` runner input and `chatgpt.modes.set/get` still work for
existing callers. New generic work should use `experience` plus
`configuration`, because Chat and Work expose different nested axes and strict
postcondition verification. Focused consultations use
