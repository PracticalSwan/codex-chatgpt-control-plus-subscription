...
visible surface-control SDK for Chat and Work. Package identities, imports,
existing backend commands, and workflow fields remain available. This fork
keeps its GPT-5.6 Sol High focused consultation in the same plugin instead of
shipping the upstream Pro-consult alias.
...
The preferred surface-neutral workflow is `chatgpt-delegate`. Focused
planning, research, reasoning, and second-opinion consultations use
`chatgpt-gpt-5-6-high-consult`, which preserves GPT-5.6 Sol at High
Intelligence and never falls back to Pro.
